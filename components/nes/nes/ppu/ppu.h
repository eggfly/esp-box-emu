...
private:
  ...
  uint32_t cycles; // total PPU cycles
  uint32_t frames; // total frames rendered
...
public:
  ...
  void setFramebuff   (u8* framebuffer);
...
public:
  uint32_t             _scancycle() const { return this->scan.cycle; }
  uint32_t             _scanline()  const { return this->scan.line;  }
