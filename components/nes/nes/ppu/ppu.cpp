...
uint32_t PPU::getNumFrames() const { return this->frames; }
...
void PPU::setFramebuff   (u8* fb) { this->framebuffer = fb;     }
...
void PPU::spr_fetch() {
  if (this->scan.cycle == 0) {
    ...
    uint32_t oam2_addr = 0; // from 0 - 32

    for (uint32_t sprite = 0; sprite < 64; sprite++) {
  ...
  if (in_range(this->scan.cycle, 257, 320)) {
    ...
    const uint32_t sprite = (this->scan.cycle - 257) / 8;
    ...
    uint32_t spr_row = this->scan.line - y_pos - 1;
    const uint32_t sprite_height = this->reg.ppuctrl.H ? 16 : 8;
...
PPU::Pixel PPU::get_bgr_pixel() {
  ...
  uint32_t pixel_type = (nth_bit(this->bgr.shift.tile[1], 15 - this->reg.x) << 1)
                  | (nth_bit(this->bgr.shift.tile[0], 15 - this->reg.x) << 0);

  uint32_t palette = (nth_bit(this->bgr.shift.at[1], 7 - this->reg.x) << 1)
...
PPU::Pixel PPU::get_spr_pixel(PPU::Pixel& bgr_pixel) {
  ...
  for (uint32_t sprite = 0; sprite < 8; sprite++) {
    // Read sprite data
    u8 y_pos      = this->oam2[sprite * 4 + 0];
    u8 tile_index = this->oam2[sprite * 4 + 1];
    union {
...
void PPU::cycle() {
  ...
  if (this->scan.line < 240 || this->scan.line == 261) {
    ...
    const uint32_t x = (this->scan.cycle - 2);
    const uint32_t y = this->scan.line;
    ...
    if (x < 256 && y != 261) {
      ...
      const uint32_t offset = (256 * 4 * y) + (4 * x);
